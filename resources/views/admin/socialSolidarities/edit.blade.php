...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.socialSolidarity.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.social-solidarities.update", [$socialSolidarity->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="title">{{ trans('cruds.socialSolidarity.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $socialSolidarity->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title_ar') ? 'is-invalid' : '' }}" type="text" name="title_ar" id="title_ar" value="{{ old('title_ar', $socialSolidarity->title_ar) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $socialSolidarity->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description_ar') ? 'is-invalid' : '' }}" name="description_ar" id="description_ar">{{ old('description_ar', $socialSolidarity->description_ar) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $socialSolidarity->date) }}">
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control  {{ $errors->has('donation_type') ? 'is-invalid' : '' }}" name="donation_type_id" id="donation_type_id">
                    @foreach($donation_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('donation_type_id') ? old('donation_type_id') : $socialSolidarity->donation_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control  {{ $errors->has('team_id') ? 'is-invalid' : '' }}" name="team_id" id="team_id">
                    @foreach($charity as $id => $entry)
                        <option value="{{ $id }}" {{ (old('team_id') ? old('team_id') : $socialSolidarity->team->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('target_amount') ? 'is-invalid' : '' }}" type="number" name="target_amount" id="target_amount" value="{{ old('target_amount', $socialSolidarity->target_amount) }}" step="1">
              ...
              <input type="hidden" value="<?php echo $socialSolidarity->active;?>" id="update_status_val" name="active">  
             <div class="form-group">
                <label class="required">{{ trans('cruds.socialSolidarity.fields.active') }}</label>
                <div class="custom-control custom-switch custom-switch-success">
                <input type="checkbox" class="custom-control-input approved_status_update" <?php echo ($socialSolidarity->active==1 ? 'checked' : null)?>  id="customSwitch2_<?php echo $socialSolidarity->id;?>" />
                <label class="custom-control-label" for="customSwitch2_<?php echo $socialSolidarity->id;?>">
