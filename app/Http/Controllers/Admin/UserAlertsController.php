...
class UserAlertsController extends Controller
{
    public function index(Request $request)
    {
        ...
        $userId = Auth::user()->id;
        DB::table('customer_customer_alert')->where(['read' => 0, 'customer_id' => $userId])->update(array('read' => 1));
        if ($request->ajax()) {
            $query = CustomerAlert::with(['users'])->select(sprintf('%s.*', (new CustomerAlert())->table));
    ...
    {
        $userAlert = CustomerAlert::create($request->all());
        $users = Customer::get();
        // sendNotifictionTopic($request->alert_text,$request->alert_link,'oman-charity-app'); 
        foreach ($users as $u) {
            DB::table('customer_customer_alert')->insert(
                array(
                    'customer_alert_id'     =>   $userAlert->id,
                    'customer_id'   =>  $u->id,
                    'read'   => 0
                )
            );
            if ($u->otp != '' && $u->is_notify == 1) {
                sendNotifiction($request->alert_text, $request->alert_link, $u->otp);
            }
        }
        return redirect()->route('admin.user-alerts.index');
    }

    public function show(CustomerAlert $userAlert)
    {
        abort_if(Gate::denies('user_alert_show'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        $noti_id = $userAlert->id;
        $userId = Auth::user()->id;
        DB::table('customer_customer_alert')->where(['customer_alert_id' => $noti_id, 'read' => 0, 'customer_id' => $userId])->update(array('read' => 1));
        $userAlert->load('users');
        return view('admin.userAlerts.show', compact('userAlert'));
    }

    public function destroy(CustomerAlert $userAlert)
    {
    ...
    {
        // return $request;
        CustomerAlert::whereIn('id', $request->ids)->delete();
